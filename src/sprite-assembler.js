...
function extractDefs(svgContent) {
  ...
  let $newSvg = cheerio.load(`<svg ${formatAttrs($svg('svg').attr())}><defs /></svg>`, {
    xmlMode: true,
  });
  ...
  $svg('symbol').each((_, element) => {
    ...
    const extractedRefIds = $svg('[id]', $symbol)
      .filter((_, elementWithId) => $symbol.html().includes(`#${$svg(elementWithId).attr('id')}`))
      .map((_, referencedEl) => {
...
module.exports = class SpriteGenerator extends Plugin {
  ...
  processFile(filePath, meta) {
    if (filePath.endsWith('.svg')) {
      ...
      this.usageMeta = JSON.parse(fs.readFileSync(filePath, { encoding: 'utf-8' }));
  ...
  emit() {
    ...
    Object.keys(this.usageMeta).forEach((symbolName) => {
      ...
      if (filePath) {
        ...
        consoleUI.warn(`missing icon for ${symbolName} (${this.usageMeta[symbolName]})`);
      }
    });
    contents.push('</svg>');
